-- Final consolidation migration to fix all remaining database function issues
-- This migration ensures all functions have consistent return types and no ambiguity

-- Drop all existing competitor analysis functions to avoid conflicts
...
-- Recreate get_competitor_share_of_voice with standardized TEXT return types
...
BEGIN
    ...
    competitor_stats AS (
        SELECT 
            ...
            COUNT(CASE WHEN car.is_mentioned THEN 1 END)::INTEGER AS total_voice_mentions,
            AVG(CASE WHEN car.is_mentioned THEN car.rank_position END)::DECIMAL AS avg_rank_position,
    ...
    SELECT 
        cs.competitor_id,
        cs.competitor_name::TEXT,
        cs.competitor_domain::TEXT,
        cs.total_analyses,
...
-- Recreate get_competitive_gap_analysis with standardized TEXT return types
...
-- Recreate analyze_competitor_mentions with standardized TEXT return types
...
-- Update function signatures in database types if needed
-- Note: These comments help with TypeScript type generation
COMMENT ON FUNCTION beekon_data.get_competitor_share_of_voice IS 'Returns competitor share of voice data with standardized TEXT types';
COMMENT ON FUNCTION beekon_data.get_competitive_gap_analysis IS 'Returns competitive gap analysis with standardized TEXT types';
COMMENT ON FUNCTION beekon_data.analyze_competitor_mentions IS 'Analyzes competitor mentions with standardized TEXT types';

-- Verify that all functions now return consistent types
-- This can be tested with: SELECT * FROM information_schema.routines WHERE routine_name LIKE '%competitor%';